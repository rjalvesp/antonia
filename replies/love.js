...
module.exports = {
  "!love": (msg) => {
    ...
    const isAzriel = R.includes(pickAuthorName(msg), [
      "AZRIEL",
      "SEÑOR CROQUETAS",
    ]);
    const commandMember = pickCommandMember(msg);
    if (commandMember.toLowerCase() === "croquetas" && isAzriel) {
    ...
    msg.channel.send(
      `Hay un ${value}% de probabilidades entre ${author} y ${commandMember}`
    );
  },
};
